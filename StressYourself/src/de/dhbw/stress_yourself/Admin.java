...
/**
 * Class Admin creates a JPanel with configuration components on it - User
 * management: - create / delete user - change password - Test management: -
 * activate / deactivate modules - change given time for modules - change module
 * order - change difficulty of the test
 * 
...
public class Admin {
	...
	private JButton btnBack = new JButton(
			"Quit Admin Area And Save All Changes");
	...
	/**
	 * Actionlistener for all buttons - checks which button is the caller and
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource().equals(btnActivateModule)) {
				if (!lAvailableModules.isSelectionEmpty()) {
					dlActiveModules.addElement(lAvailableModules
							.getSelectedValue() + " ; " + tfTime.getText());
				...
				if (!lActiveModules.isSelectionEmpty()) {
					dlAvailableModules.addElement(lActiveModules
							.getSelectedValue().substring(
									0,
									lActiveModules.getSelectedValue().indexOf(
											" ; ")));
				...
				if (tfUsername.getText().length() != 0
					...
					if (pfPassword.getPassword().length == 0) {
						pfPassword.setBorder(BorderFactory
								.createLineBorder(Color.RED));
					}
				}
				...
				if (Integer.parseInt(tfTime.getText()) > 60) {
					tfTime.setText(String.valueOf(Integer.parseInt(tfTime
							.getText()) - 30));
				}
				...
				for (int i = 0; i < dlActiveModules.getSize(); i++) {
					dlAvailableModules
							.addElement(dlActiveModules.get(i).substring(0,
									dlActiveModules.get(i).indexOf(" ; ")));
				}
				...
				for (int i = 0; i < dlAvailableModules.getSize(); i++) {
					dlActiveModules.addElement(dlAvailableModules.get(i)
							+ " ; " + tfTime.getText());
				}
	...
	/**
	 * Constructor - creates an object of the class Admin and gets information
	 * about users, parameters and mainapplication
	 * 
	 * @param users
	 ...
	 * 
	 * @return JPanel with the named contents on it
	 */
	...
	/**
	 * Creates a JPanel with scrollpane on it with the names of the registered
	 * users
	 */
	...
	/**
	 * Creates the status panel with the button save and go back on it
	 */
	...
	private void initColors() {
		...
		// Initialize colors
		backgroundColor = new Color(aPanel.getBackground().getRed() - 9, aPanel
				.getBackground().getGreen() - 9, aPanel.getBackground()
				.getBlue() - 9);
	}

	/**
	 * Creates the panel "User Management" with all components which are needed
	 * to manage users
	 */
	...
	/**
	 * Creates the panel "Test Management" with all components which are needed
	 * to manage test routines
	 */
			...
			for (int i = 0; i < params.getAvailableModules().size(); i++) {
				...
				for (int b = 0; b < params.getConfiguration().size(); b++) {
					...
					if (params
							...
							.get(b)
							.getName()
							.equals(params.getAvailableModules().get(i)
									.getName())) {
						exists = true;
						break;
					}
				...
				if (!exists) {
					dlAvailableModules.addElement(params.getAvailableModules()
							.get(i).getName());
					exists = false;
				}
			}
			for (int i = 0; i < params.getConfiguration().size(); i++) {
				dlActiveModules.add(i, params.getConfiguration().get(i)
						.getName()
	...
	/**
	 * Moves the selected module 1 place up or down depending on the param
	 * direction
	 * 
	 * @param direction
	...
	/**
	 * Class SetMaxText - with this class you are able to set the maxlength of
	 * an JTextField / JPasswordField in a very easy way. - use:
	 * someTextField.setDocument(new SetMaxText(int Max));
	 */
	...
	/**
	 * Creates an user or admin User or admin will be saved after closing the
	 * programm. If the programm chrashes the changes are lost.
	 * 
	 * @return true if user or admin was created / false if some param is not ok
	 */
	...
	/**
	 * Deletes a user or admin User or admin will be deleted after closing the
	 * programm. If the programm chrashes the changes are lost!
	 * 
	...
	/**
	 * Changes the password of an user or admin. Password will be saved after
	 * returning closing the programm. If the programm chrashes the changes are
	 * lost!
	 * 
	 * @return true if the password was changed in temp user information.
	 */
	...
	/**
	 * Save configuration temporarely Will be saved after closing the programm.
		...
		for (int i = 0; i < dlActiveModules.size(); i++) {
			for (int x = 0; x < params.getAvailableModules().size(); x++) {
				...
				if (dlActiveModules
						.getElementAt(i)
						.substring(0,
								dlActiveModules.getElementAt(i).indexOf(" ; "))
						.equals(params.getAvailableModules().get(x).getName())) {
					params.getAvailableModules()
							.get(x)
							.setTime(
									Integer.parseInt(dlActiveModules
											.getElementAt(i)
											.substring(
													dlActiveModules
															.getElementAt(i)
	...
	/**
	 * Initialize the components so that if an admin calls the admin area twice
	 * the components are reloaded
	 */
	...
	/**
	 * Keylistener for letters only - used on textfield "Username"
	 */
		...
		public void keyReleased(KeyEvent e) {
			...
			if (e.getSource().equals(tfUsername)) {
				if ((e.getKeyCode() != (char) 8 && e.getKeyChar() < (char) 65)
						...
						|| e.getKeyChar() > (char) 122
						&& e.getKeyChar() < (char) 255) {
					tfUsername.setText(tfUsername.getText().substring(0,
							tfUsername.getText().length() - 1));
				}
				...
				if ((e.getKeyChar() != (char) 8 && e.getKeyChar() < (char) 48)
						...
						|| e.getKeyChar() > (char) 122
						&& e.getKeyChar() < (char) 255) {
					pfPassword.setText(String.valueOf(pfPassword.getPassword())
							.substring(0, pfPassword.getPassword().length - 1));
				}
			}
